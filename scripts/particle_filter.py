...
x_func = s.Piecewise(
    (0.20 * t_var + 0.8, t_var <= 1),
    (0.45 * t_var + 0.55, t_var <= 2),
    (-0.12 * t_var + 1.69, t_var <= 3),
    (1.33, True),
)
y_func = s.Piecewise(
    (1.15 * t_var + 0.3, t_var <= 1),
    (1.45, t_var <= 2),
    (-1.3 * t_var + 4.05, t_var <= 3),
    (0.15, True),
)
...
class ParticleFilter:
    def __init__(self):
        ...
        self.aStarPathMap = np.vectorize(lambda x: (1 if x > 0.155 and x < 0.6 else 0))(
            self.closestMap
        )
        ...
        self.particle_cloud = np.array(
            [Particle(Pose(), 0) for _ in range(self.num_particles)]
        )
        ...
        self.find_poses = np.zeros(
            shape=(2, self.find_num_particles)
        )  # x, y of poses only
        ...
        self.first_pf_cycle = (
            True  # do first pf cycle regardless of whether the bot moves
        )
    ...
    def scatter_particle_cloud(self):
        ...
        self.find_yaws[:] = np.random.uniform(
            low=0, high=2 * np.pi, size=self.find_num_particles
        )
        self.find_weights[:] = np.ones(shape=self.find_num_particles)

        coords = self.ib_indices[
            :,
            np.random.randint(
                low=0, high=self.num_ib_indices, size=self.find_num_particles
            ),
        ]
        xs = coords[0]
        ys = coords[1]
    ...
    def resample_particles(self):
        ...
        sample = np.random.choice(
            np.arange(num_particles), size=num_particles, p=weights
        )
    ...
    def robot_scan_received(self, data):
        ...
        if len(self.particle_cloud) > 0:
            ...
            if (
                ...
                error = 0.25
                ang_vel = np.arctan2(next_node[1], next_node[0]) - get_yaw_from_pose(
                    self.robot_estimate
                )
                ...
                print(
                    "yaws:",
                    np.arctan2(next_node[1], next_node[0]),
                    " ",
                    get_yaw_from_pose(self.robot_estimate),
                )
                ...
                self.pub_cmd_vel.publish(
                    Twist(
                        linear=Vector3(error * 2 * lin_vel, 0, 0),
                        angular=Vector3(0, 0, error * ang_vel),
                    )
                )
    ...
    def update_particle_weights_with_measurement_model(self, data):
        ...
        in_bound = (
            (norm_xs >= 0) & (norm_xs < width) & (norm_ys >= 0) & (norm_ys < height)
        )
        ...
        prob_ds = np.exp(-(dists**2) / (2 * (sigma_hit**2))) / (
        ...
        for i in range(num_particles):
            ...
            # compute likelihood probabilities of each distance
            prob_ds = np.exp(-pow(dists, 2) / (2 * pow(sigma_hit, 2))) / (
                sigma_hit * np.sqrt(2 * np.pi)
            )
    ...
    while not rospy.is_shutdown():
        ...
        if not is_approx:
            ...
            result = minimize_scalar(
                d_curr_lam, bounds=(curr_t + 0.01, curr_t + 1), method="bounded"
            )
            t = result.x
        ...
        pub_cmd_vel.publish(
            Twist(
                linear=Vector3(0.5 * lin_error, 0, 0),
                angular=Vector3(0, 0, 0.5 * ang_error),
            )
        )
        print("MOVE: ", time.time() - start)
    rospy.spin()
